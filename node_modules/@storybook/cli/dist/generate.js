...
`),logger6.info(`\u26A0\uFE0F Storybook automigrations are based on AST parsing and it's possible that your ${import_chalk4.default.blue(mainConfigPath)} file contains a non-standard format (e.g. your export is not an object) or that there was an error when parsing dynamic values (e.g. "require" calls, or usage of environment variables). When your main config is non-standard, automigrations are unfortunately not possible. Please follow the instructions given previously and follow the documentation to make the updates manually.`)}},getAddonNames=mainConfig=>(mainConfig.addons||[]).map(addon=>{let name="";if(typeof addon=="string"?name=addon:typeof addon=="object"&&(name=addon.name),!name.startsWith("."))return name.replace(/\/dist\/.*/,"").replace(/\.[mc]?[tj]?s[x]?$/,"").replace(/\/register$/,"").replace(/\/manager$/,"").replace(/\/preset$/,"")}).filter(Boolean);var logger7=console,checkWebpack5Builder=async({configDir,packageManager})=>{var _a;let{mainConfig,storybookVersion}=await getStorybookData({configDir,packageManager});if(import_semver8.default.lt(storybookVersion,"6.3.0"))return logger7.warn(import_ts_dedent7.default`
      ...
      `.trim()),null;if(import_semver8.default.gte(storybookVersion,"7.0.0"))return null;if(!mainConfig)return logger7.warn("Unable to find storybook main.js config"),null;let builder=(_a=mainConfig.core)==null?void 0:_a.builder;return builder&&builder!=="webpack4"?(logger7.info(`Found builder ${builder}, skipping`),null):{storybookVersion}};var logger8=console,webpack5={id:"webpack5",async check({configDir,packageManager}){var _a;let webpackVersion=(await packageManager.retrievePackageJson()).dependencies.webpack,webpackCoerced=(_a=import_semver9.default.coerce(webpackVersion))==null?void 0:_a.version;if(!webpackCoerced||import_semver9.default.lt(webpackCoerced,"5.0.0")||import_semver9.default.gte(webpackCoerced,"6.0.0"))return null;let builderInfo=await checkWebpack5Builder({configDir,packageManager});return builderInfo?{webpackVersion,...builderInfo}:null},prompt({webpackVersion}){let webpackFormatted=import_chalk6.default.cyan(`webpack ${webpackVersion}`);return import_ts_dedent8.dedent`
...
`),(dependenciesToRemove.includes("@storybook/builder-webpack4")||dependenciesToRemove.includes("@storybook/manager-webpack4"))&&(disclaimer=import_ts_dedent17.default`\n\n
      ...
      To learn more about migrating from Webpack4, see: ${import_chalk14.default.yellow("https://github.com/storybookjs/storybook/blob/next/MIGRATION.md#webpack4-support-discontinued")}`),metaFramework==="nextjs"&&(dependenciesToRemove.includes("storybook-addon-next-router")&&(migrationSteps+=`- Migrate the usage of the ${import_chalk14.default.cyan("storybook-addon-next-router")} addon to use the APIs from the ${import_chalk14.default.magenta("@storybook/nextjs")} framework package instead. Follow the instructions below.`),frameworkPackage==="@storybook/react-vite"?disclaimer=import_ts_dedent17.default`\n\n
        ...
        `:frameworkPackage==="@storybook/nextjs"&&(disclaimer=import_ts_dedent17.default`\n\n
        ...
        `)),metaFramework==="sveltekit"&&(frameworkPackage==="@storybook/svelte-webpack5"?disclaimer=import_ts_dedent17.default`\n\n
...
`,disclaimer=import_ts_dedent17.default`\n\n
...
`)}
    ...
    `}};var import_chalk16=__toESM(require("chalk")),import_ts_dedent19=require("ts-dedent"),import_path12=require("path"),import_fs_extra11=__toESM(require("fs-extra")),import_globby2=__toESM(require("globby")),MDX1_STYLE_START=/<style>{`/g,MDX1_STYLE_END=/`}<\/style>/g,MDX1_COMMENT=/<!--(.+)-->/g,MDX1_CODEBLOCK=/(?:\n~~~(?:\n|.)*?\n~~~)|(?:\n```(?:\n|.)*?\n```)/g,fixMdxStyleTags=mdx=>mdx.replace(MDX1_STYLE_START,"<style>\n  {`").replace(MDX1_STYLE_END,"  `}\n</style>"),fixMdxComments=mdx=>{let codeblocks=mdx.matchAll(MDX1_CODEBLOCK);return mdx.split(MDX1_CODEBLOCK).map(v=>v.replace(MDX1_COMMENT,(original,group)=>`{/*${group}*/}`)).reduce((acc,item,i)=>{let next=codeblocks.next();return next.done?acc+item:acc+item+next.value[0]},"")},logger16=console,mdx1to2={id:"mdx1to2",async check(){let storiesMdxFiles=await(0,import_globby2.default)("./!(node_modules)**/*.(story|stories).mdx");return storiesMdxFiles.length?{storiesMdxFiles}:void 0},prompt({storiesMdxFiles}){return import_ts_dedent19.dedent`
    ...
    `;return value?import_ts_dedent20.dedent`
...
`);return import_ts_dedent21.dedent`
    ...
    `},async run(){logger18.info(),await generateStorybookBabelConfigInCWD()}};var import_ts_dedent24=require("ts-dedent"),import_semver19=__toESM(require("semver")),import_chalk21=__toESM(require("chalk")),import_prompts4=__toESM(require("prompts"));var angularBuilders={id:"angular-builders",async check({packageManager,configDir}){var _a;let packageJSON=await packageManager.retrievePackageJson();if(isNxProject(packageJSON))return null;let angularVersion=(await packageManager.getAllDependencies())["@angular/core"],angularCoerced=(_a=import_semver19.default.coerce(angularVersion))==null?void 0:_a.version;if(!angularCoerced)return null;if(import_semver19.default.lt(angularCoerced,"14.0.0"))throw new Error(import_ts_dedent24.dedent`
      ...
      `);let angularJSON=new AngularJSON,{hasStorybookBuilder}=angularJSON;if(hasStorybookBuilder||!angularJSON.rootProject&&Object.keys(angularJSON.projects).length>1)return null;let{mainConfig}=await getStorybookData({configDir,packageManager});return{mainConfig,packageManager}},prompt(){return import_ts_dedent24.dedent`
...
`);return import_ts_dedent29.default`
    ...
    To learn more about this change, see: ${import_chalk27.default.yellow("https://github.com/storybookjs/storybook/blob/next/MIGRATION.md#mdx-docs-files")}
  `},async run({dryRun,mainConfigPath,result:{nextStoriesEntries}}){logger22.info(import_ts_dedent29.default`✅ Setting 'stories' config:
...
`)}
            `.trim(),{borderStyle:"round",padding:1,borderColor:"#F1618C"})),process.exit(1)),choices.length===1?[selectedTemplate]=choices:(logger26.info((0,import_boxen3.default)(import_ts_dedent30.dedent`
...
yarn storybook`):`Recreate your setup, then ${import_chalk28.default.yellow("npx storybook@latest init")}`;logger26.info((0,import_boxen3.default)(import_ts_dedent30.dedent`
