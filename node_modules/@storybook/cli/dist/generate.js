...
`);return import_ts_dedent20.dedent`
    ...
    `},async run({result}){let angularJSON=new AngularJSON,{packageManager}=result,{useCompoDoc}=await(0,import_prompts4.default)({type:"confirm",name:"useCompoDoc",message:"Have you set up compodoc in Storybook previously?"}),angularProjectName=await angularJSON.getProjectName();angularJSON.addStorybookEntries({angularProjectName,storybookFolder:".storybook",useCompodoc:useCompoDoc,root:"."}),angularJSON.write(),packageManager.addScripts({storybook:`ng run ${angularProjectName}:storybook`,"build-storybook":`ng run ${angularProjectName}:build-storybook`})}};var import_ts_dedent24=require("ts-dedent"),import_semver20=__toESM(require("semver")),import_chalk22=__toESM(require("chalk"));var angularBuildersMultiproject={id:"angular-builders-multiproject",promptOnly:!0,async check({packageManager,configDir}){var _a;let packageJSON=packageManager.retrievePackageJson();if(isNxProject(packageJSON))return null;let angularVersion=packageManager.getAllDependencies()["@angular/core"],angularCoerced=(_a=import_semver20.default.coerce(angularVersion))==null?void 0:_a.version;if(!angularCoerced||import_semver20.default.lt(angularCoerced,"14.0.0"))return null;let angularJSON=new AngularJSON,{hasStorybookBuilder}=angularJSON;return hasStorybookBuilder||angularJSON.rootProject||Object.keys(angularJSON.projects).length===1?null:{}},prompt(){return import_ts_dedent24.dedent`
    In Storybook 6.4 we have deprecated calling Storybook directly (npm run storybook) for Angular. In Storybook 7.0, we've removed it entirely. Instead you have to set up the Storybook builder in your ${import_chalk22.default.yellow("angular.json")} and execute ${import_chalk22.default.yellow("ng run <your-project>:storybook")} to start Storybook. 
    
    ❌ Your Angular workspace uses multiple projects defined in the ${import_chalk22.default.yellow("angular.json")} file and we were not able to detect a root project. Therefore we are not able to automigrate to use Angular Storybook builder. Instead, please visit ${import_chalk22.default.yellow("https://github.com/storybookjs/storybook/tree/next/code/frameworks/angular")} to do the migration manually.
    `}};var allFixes=[nodeJsRequirement,cra5,webpack5,angular12,vue3,eslintPlugin,builderVite,sbBinary,sbScripts,newFrameworks,removedGlobalClientAPIs,mdx1to2,mdxgfm,autodocsTrue,addReact,missingBabelRc,angularBuildersMultiproject,angularBuilders],initFixes=[missingBabelRc,eslintPlugin];var import_os=require("os"),ansiRegex=({onlyFirst=!1}={})=>{let pattern=["[\\u001B\\u009B][[\\]()#;?]*(?:(?:(?:(?:;[-a-zA-Z\\d\\/#&.:=?%@~_]+)*|[a-zA-Z\\d]+(?:;[-a-zA-Z\\d\\/#&.:=?%@~_]*)*)?\\u0007)","(?:(?:\\d{1,4}(?:;\\d{0,4})*)?[\\dA-PR-TZcf-ntqry=><~]))"].join("|");return new RegExp(pattern,onlyFirst?void 0:"g")},cleanLog=str=>str.replace(ansiRegex(),"").replace(/╮│/g,`\u256E
\u2502`).replace(/││/g,`\u2502
\u2502`).replace(/│╰/g,`\u2502
\u2570`).replace(/⚠️ {2}failed to check/g,`${import_os.EOL}\u26A0\uFE0F  failed to check`);var import_chalk23=__toESM(require("chalk")),import_boxen=__toESM(require("boxen")),import_core_common6=require("@storybook/core-common"),import_ts_dedent25=__toESM(require("ts-dedent"));var import_semver21=__toESM(require("semver"));function hasMultipleVersions(versions){return versions.find(v=>{let major=import_semver21.default.major(v);if(major===0){let minor=import_semver21.default.minor(v);if(minor===0){let patch=import_semver21.default.patch(v);return versions.some(v2=>import_semver21.default.patch(v2)!==patch)}return versions.some(v2=>import_semver21.default.minor(v2)!==minor)}return versions.some(v2=>import_semver21.default.major(v2)!==major)})}var messageDivider=`

`,segmentDivider=`

\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500

`;function getGlossaryMessages(fixSummary,fixResults,logFile){let messages=[];return fixSummary.succeeded.length>0&&(messages.push(import_chalk23.default.bold("Successful migrations:")),messages.push(fixSummary.succeeded.map(m=>import_chalk23.default.green(m)).join(", "))),Object.keys(fixSummary.failed).length>0&&(messages.push(import_chalk23.default.bold("Failed migrations:")),messages.push(Object.entries(fixSummary.failed).map(([id,error])=>`${import_chalk23.default.redBright(id)}:
${error}`).join(`
`)),messages.push(`You can find the full logs in ${import_chalk23.default.cyan(logFile)}`)),fixSummary.manual.length>0&&(messages.push(import_chalk23.default.bold("Manual migrations:")),messages.push(fixSummary.manual.map(m=>fixResults[m]==="manual_succeeded"?import_chalk23.default.green(m):import_chalk23.default.blue(m)).join(", "))),fixSummary.skipped.length>0&&(messages.push(import_chalk23.default.bold("Skipped migrations:")),messages.push(fixSummary.skipped.map(m=>import_chalk23.default.cyan(m)).join(", "))),messages}function getMigrationSummary({fixResults,fixSummary,logFile,installationMetadata}){let messages=[];messages.push(getGlossaryMessages(fixSummary,fixResults,logFile).join(messageDivider)),messages.push(import_ts_dedent25.default`If you'd like to run the migrations again, you can do so by running '${import_chalk23.default.cyan("npx storybook@next automigrate")}'
...
The following migrations are available: ${availableFixes}`)},automigrate=async({fixId,fixes:inputFixes,dryRun,yes,useNpm,packageManager:pkgMgr,list,configDir:userSpecifiedConfigDir,renderer:rendererPackage,skipInstall}={})=>{if(list)return logAvailableMigrations(),null;let selectedFixes=inputFixes||allFixes,fixes=fixId?selectedFixes.filter(f=>f.id===fixId):selectedFixes;if(fixId&&fixes.length===0)return logger19.info(`\u{1F4ED} No migrations found for ${import_chalk24.default.magenta(fixId)}.`),logAvailableMigrations(),null;augmentLogsToFile(),logger19.info("\u{1F50E} checking possible migrations..");let{fixResults,fixSummary}=await runFixes({fixes,useNpm,pkgMgr,userSpecifiedConfigDir,rendererPackage,skipInstall,dryRun,yes});Object.values(fixResults).some(r=>r==="failed"||r==="check_failed")?await(0,import_fs_extra11.move)(TEMP_LOG_FILE_PATH,(0,import_path13.join)(process.cwd(),LOG_FILE_NAME),{overwrite:!0}):await(0,import_fs_extra11.remove)(TEMP_LOG_FILE_PATH);let installationMetadata=await JsPackageManagerFactory.getPackageManager({force:pkgMgr}).findInstallations(["@storybook/*","storybook"]);return logger19.info(),logger19.info(getMigrationSummary({fixResults,fixSummary,logFile:LOG_FILE_PATH,installationMetadata})),logger19.info(),cleanup(),fixResults};async function runFixes({fixes,dryRun,yes,useNpm,pkgMgr,userSpecifiedConfigDir,rendererPackage,skipInstall}){var _a;useNpm&&(useNpmWarning(),pkgMgr="npm");let packageManager=JsPackageManagerFactory.getPackageManager({force:pkgMgr}),{configDir:inferredConfigDir,mainConfig:mainConfigPath,version:storybookVersion}=(0,import_core_common7.getStorybookInfo)(packageManager.retrievePackageJson(),userSpecifiedConfigDir);if(!(storybookVersion&&((_a=import_semver22.default.coerce(storybookVersion))==null?void 0:_a.version)))return logger19.info(import_ts_dedent26.default`
      ...
      `),{preCheckFailure:"undetected_sb_version"};let configDir=userSpecifiedConfigDir||inferredConfigDir||".storybook";try{await(0,import_core_common7.loadMainConfig)({configDir})}catch(err){return err.message.includes("No configuration files have been found")?(logger19.info(import_ts_dedent26.default`[Storybook automigrate] Could not find or evaluate your Storybook main.js config directory at ${import_chalk24.default.blue(configDir)} so the automigrations will be skipped. You might be running this command in a monorepo or a non-standard project structure. If that is the case, please rerun this command by specifying the path to your Storybook config directory via the --config-dir option.`),{preCheckFailure:"mainjs_not_found"}):(logger19.info(import_ts_dedent26.default`[Storybook automigrate] ❌ Failed trying to evaluate ${import_chalk24.default.blue(mainConfigPath)} with the following error: ${err.message}`),logger19.info("Please fix the error and try again."),{preCheckFailure:"mainjs_evaluation_error"})}let fixResults={},fixSummary={succeeded:[],failed:{},manual:[],skipped:[]};for(let i=0;i<fixes.length;i+=1){let f=fixes[i],result;try{result=await f.check({packageManager,configDir,rendererPackage})}catch(error){logger19.info(`\u26A0\uFE0F  failed to check fix ${import_chalk24.default.bold(f.id)}`),logger19.error(`
${error.stack}`),fixSummary.failed[f.id]=error.message,fixResults[f.id]="check_failed"}if(result){logger19.info(`
\u{1F50E} found a '${import_chalk24.default.cyan(f.id)}' migration:`);let message=f.prompt(result);logger19.info((0,import_boxen2.default)(message,{borderStyle:"round",padding:1,borderColor:"#F1618C",title:f.promptOnly?"Manual migration detected":"Automigration detected"}));let runAnswer;try{if(dryRun)runAnswer={fix:!1};else if(yes)runAnswer={fix:!0},f.promptOnly&&(fixResults[f.id]="manual_succeeded",fixSummary.manual.push(f.id));else if(f.promptOnly){fixResults[f.id]="manual_succeeded",fixSummary.manual.push(f.id),logger19.info();let{shouldContinue}=await(0,import_prompts5.default)({type:"toggle",name:"shouldContinue",message:"Select continue once you have made the required changes, or quit to exit the migration process",initial:!0,active:"continue",inactive:"quit"},{onCancel:()=>{throw new Error}});if(!shouldContinue){fixResults[f.id]="manual_skipped";break}}else runAnswer=await(0,import_prompts5.default)({type:"confirm",name:"fix",message:`Do you want to run the '${import_chalk24.default.cyan(f.id)}' migration on your project?`,initial:!0},{onCancel:()=>{throw new Error}})}catch{break}if(!f.promptOnly)if(runAnswer.fix)try{await f.run({result,packageManager,dryRun,mainConfigPath,skipInstall}),logger19.info(`\u2705 ran ${import_chalk24.default.cyan(f.id)} migration`),fixResults[f.id]="succeeded",fixSummary.succeeded.push(f.id)}catch(error){fixResults[f.id]="failed",fixSummary.failed[f.id]=error.message,logger19.info(`\u274C error when running ${import_chalk24.default.cyan(f.id)} migration`),logger19.info(error),logger19.info()}else fixResults[f.id]="skipped",fixSummary.skipped.push(f.id)}else fixResults[f.id]=fixResults[f.id]||"unnecessary"}return{fixResults,fixSummary}}var logger20=console,installStorybook=(projectType,packageManager,options)=>{let npmOptions={installAsDevDependencies:!0,skipInstall:options.skipInstall},packageJson;try{packageJson=packageManager.readPackageJson()}catch{}let language=detectLanguage(packageJson),pnp=detectPnp(),generatorOptions={language,builder:options.builder||detectBuilder(packageManager,projectType),linkable:!!options.linkable,pnp:pnp||options.usePnp};return(async()=>{switch(projectType){case"REACT_SCRIPTS":return REACT_SCRIPTS_default(packageManager,npmOptions,generatorOptions).then(commandLog('Adding Storybook support to your "Create React App" based project'));case"REACT":return REACT_default(packageManager,npmOptions,generatorOptions).then(commandLog(`Adding Storybook support to your "React" app
...
For more information visit:`,import_chalk25.default.cyan("https://storybook.js.org")),projectType==="ANGULAR"?(logger20.log(`
To run your Storybook, type:
`),codeLog([`ng run ${installResult.projectName}:storybook`])):projectType==="REACT_NATIVE"?(logger20.log(),logger20.log(import_chalk25.default.yellow(`NOTE: installation is not 100% automated.
...
`);return import_ts_dedent27.default`
...
`)}
            `.trim(),{borderStyle:"round",padding:1,borderColor:"#F1618C"})),process.exit(1)),choices.length===1?[selectedTemplate]=choices:(logger26.info((0,import_boxen3.default)(import_ts_dedent28.dedent`
